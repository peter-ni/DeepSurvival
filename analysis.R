...
# Best
# auc_df = read_csv("Desktop/Projects/DeepSurvival/DeepSurvival/saved_models/auc_df_01_33_epochs750.csv") %>% as.data.frame()
# loss_df = read_csv("Desktop/Projects/DeepSurvival/DeepSurvival/saved_models/loss_df_01_33_epochs750.csv") %>% as.data.frame()
...
auc_plt = ggplot(data = auc_df, aes(x = Time, y = AUC)) + geom_line() + ggtitle("Average Time-dependent AUC on Cross-Validated Test Sets")
